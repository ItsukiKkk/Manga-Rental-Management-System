prompt PL/SQL Developer Export Tables for user TAOBAO@ORCL
...
(
  category_cd VARCHAR2(3) not null,
...
prompt Creating PREFECTURE...
create table PREFECTURE
(
  pre_cd   VARCHAR2(2) not null,
  pre_name VARCHAR2(10) not null
)
;
alter table PREFECTURE
  add primary key (PRE_CD);
alter table PREFECTURE
  add constraint PRE_CD_CHECK
  check (REGEXP_LIKE(pre_cd, '^\d{2}$'));
...
alter table CITY
  add constraint FK_CITY_RRECD_CD foreign key (PRE_CD)
  references PREFECTURE (PRE_CD);

prompt Creating CLERK...
create table CLERK
(
  clerk_id  VARCHAR2(3) not null,
  lastname  VARCHAR2(30) not null,
  lastkana  VARCHAR2(60) not null,
  firstname VARCHAR2(30) not null,
  firstkana VARCHAR2(60) not null,
...
alter table CUSTOMER
  add constraint DEL_FLAG_CHECK
  check (del_flag IN (0, 1));
alter table CUSTOMER
  add constraint ID_FLAG_CHECK
  check (id_flag IN (0, 1, 9));
alter table CUSTOMER
  add constraint POST_FORMAT_CHECK
  check (REGEXP_LIKE(post, '^[0-9]{7}$'));
...
(
  ...
  a_phone    NUMBER(20),
  a_power    VARCHAR2(20),
  a_describe VARCHAR2(40)
)
;
alter table D_ADMIN
  add primary key (A_ID);

prompt Creating D_CLASS...
create table D_CLASS
(
  c_id         NUMBER(11) not null,
  c_classid    NUMBER(11) not null,
  c_classname  VARCHAR2(30),
  c_counsellor VARCHAR2(30)
)
;
alter table D_CLASS
  add primary key (C_ID);

prompt Creating D_DORMGRADE...
create table D_DORMGRADE
(
  g_id           NUMBER(11) not null,
  d_id           NUMBER(11) not null,
  d_dormbuilding VARCHAR2(20),
  d_grade        NUMBER(11),
  create_time    VARCHAR2(50),
  update_time    VARCHAR2(50)
)
;
alter table D_DORMGRADE
  add primary key (G_ID);

prompt Creating D_DORMITORYINFO...
create table D_DORMITORYINFO
(
  d_id           NUMBER(11) not null,
  s_dormitoryid  NUMBER(11) not null,
  d_dormbuilding VARCHAR2(20),
  d_bedtotal     VARCHAR2(20),
  d_bed          VARCHAR2(20),
  a_name         VARCHAR2(20)
)
;
alter table D_DORMITORYINFO
  add primary key (D_ID);

prompt Creating D_DORMREPAIR...
create table D_DORMREPAIR
(
  r_id           NUMBER(11) not null,
  d_id           NUMBER(11) not null,
  d_dormbuilding VARCHAR2(20) not null,
  r_name         VARCHAR2(20),
  reason         VARCHAR2(50),
  create_time    VARCHAR2(50),
  update_time    VARCHAR2(50)
...
;
alter table D_DORMREPAIR
  add primary key (R_ID);

prompt Creating D_STGRADE...
create table D_STGRADE
(
  g_id          NUMBER(11) not null,
  s_studentid   NUMBER(11) not null,
  s_name        VARCHAR2(20),
  s_grade       NUMBER(11),
  s_classid     NUMBER(11),
  s_dormitoryid NUMBER(11),
  create_time   VARCHAR2(50),
  update_time   VARCHAR2(50)
)
;
alter table D_STGRADE
  add primary key (G_ID);

prompt Creating D_STUDENT...
...
(
  ...
  s_age         NUMBER(11),
  s_phone       NUMBER(20),
  s_classid     NUMBER(11) not null,
  s_classname   VARCHAR2(20),
  s_dormitoryid NUMBER(11) not null
)
;
...
prompt Creating D_VISITOR...
create table D_VISITOR
(
  v_id           NUMBER(11) not null,
  v_name         VARCHAR2(20),
  v_phone        NUMBER(20),
  v_dormitoryid  NUMBER(11),
  v_dormbuilding VARCHAR2(20),
  create_time    VARCHAR2(50)
)
;
alter table D_VISITOR
...
prompt Creating STATISTICAL_LOG...
create table STATISTICAL_LOG
(
  statistical_id     NUMBER not null,
  statistical_name   VARCHAR2(50) not null,
  statistical_url    VARCHAR2(200) not null,
  statistical_number NUMBER default 0,
  statistical_date   DATE
)
;
alter table STATISTICAL_LOG
  add primary key (STATISTICAL_ID);

prompt Creating TITLE...
create table TITLE
(
  title_id VARCHAR2(8) not null,
  title    VARCHAR2(100) not null,
  tem_cd   VARCHAR2(13),
  ctg_cd   VARCHAR2(3) not null,
  actor    VARCHAR2(100)
)
;
alter table TITLE
...
alter table TITLE
  add constraint TEM_CD unique (TEM_CD);
alter table TITLE
  add constraint FK_CTG_CD foreign key (CTG_CD)
...
(
  title_id    VARCHAR2(8) not null,
  stock_no    VARCHAR2(3) not null,
  stock_state INTEGER default 0 not null
)
;
alter table STOCK
  add constraint PK_STOCK primary key (TITLE_ID, STOCK_NO);
alter table STOCK
  add constraint FK_TITLE_ID foreign key (TITLE_ID)
  references TITLE (TITLE_ID);
alter table STOCK
  add constraint STOCK_STATE_CHECK
  check (stock_state IN (0, 1, 9));
