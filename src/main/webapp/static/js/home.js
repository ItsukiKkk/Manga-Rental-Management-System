...
function eachIframe(element) {
    setTimeout(function() {
        ...
        for (var i = 0; i < iframes.length; i++) {
            ...
            // 先解绑之前绑定的事件
            $(idoc.documentElement).off('click', '#topMenu a');
            $(idoc.documentElement).off('click', '#toMenuBtn button');
            bindEvents(idoc, element);
        }
    }, 300);
}

// 绑定事件处理程序
function bindEvents(idoc, element) {
    // 监听iframe里的超链接点击事件
    $(idoc.documentElement).on('click', '#topMenu a', function(event) {
        event.preventDefault();
        var url = $(this).attr('href');
        // 触发左侧导航栏相应链接的点击事件
        $('#Nav a[href="' + url + '"]').trigger('click');
    });

    // 监听iframe里返回菜单一览页面的按钮点击事件
    $(idoc.documentElement).on('click', '#toMenuBtn button', function(event) {
        event.preventDefault();
        // 获取按钮的 data-url 属性值
        var clickedUrl = $(this).data('url'); 
        
        console.log("this is clickedUrl"+clickedUrl);
        // 设置菜单一览页面url
        var url = "${pageContext.request.contextPath}/topMenu";
        // 触发左侧导航栏相应链接的点击事件
        $('#Nav a[href="' + url + '"]').trigger('click');
        // 关闭当前选项卡页面
        var currentTabId = $('li.layui-this').attr('lay-id');
        console.log("this is currentTabId"+currentTabId);
        if (currentTabId && clickedUrl == currentTabId) {
            element.tabDelete('tabs', currentTabId);
        } 
        if (clickedUrl == '/rental/stockRegForm'){
			if (currentTabId == '/rental/titleRegForm' || currentTabId == '/rental/titleSelForm') {
				element.tabDelete('tabs', currentTabId);
			}
		}
    });
